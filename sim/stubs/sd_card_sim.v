...
//
//   +KDIA=<file>  +KDIB=<file>  +KDIC=<file>  +KDID=<file>  +OPTS=<file>
//
...
// floppies and the ROM image - and then a
...
localparam MAXB = 1024 * 1024;   // bytes an image may have (a .kdi is 819200)
...
initial begin
    ...
    if ($value$plusargs("KDIA=%s", fname)) load(0, fname);
    if ($value$plusargs("KDIB=%s", fname)) load(1, fname);
    if ($value$plusargs("KDIC=%s", fname)) load(2, fname);
    if ($value$plusargs("KDID=%s", fname)) load(3, fname);
    if ($value$plusargs("OPTS=%s", fname)) load(4, fname);
end
