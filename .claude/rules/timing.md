...
that that cannot happen, and this design keeps that exactly, at 40.5 MHz
instead of 30.

## The invariants

- **One internal clock.**  `clk` is the PLL's 40.5 MHz and every flop is
  on it.  The CPU's 2.5 MHz is two one-clock enables (`f1`, `f2`) at
  phases 0 and 8 of `tphase`; the Z80's is an enable at phase 0 (and 8
  in turbo); the pixel is four clocks; the timer's 2 MHz is an enable
  every twenty; the I2S bit clock is a registered output toggled by a
  counter and clocks nothing inside the chip.  The PLL's phase-shifted
  copy goes to the SDRAM pad and to nothing else.
- **Two clocks the tool knows about, and no more.**  `korvet.sdc`
  declares `clk27` (the crystal), `clk40` (the PLL output, on its pin -
...
3. If it brings its own clock (a card interface, a serial line), that
   ...
   example.  The serial mouse's 1200 baud and the ВВ51's clocks are
   enables, not clocks.
4. Build.  Read the gate's output, then the clock table in
   `tang/impl/pnr/korvet_tr_content.html`: three clocks and the derived
