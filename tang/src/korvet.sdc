//========================================================================
// Timing constraints for korvet (Korvet Nano on the Tang Nano 20K).
//========================================================================
// The design has ONE internal clock, and that is the whole point of its
// clocking: 40.5 MHz out of the PLL, everything on it, the CPU's T-state
...
// hdmi_serdes.v with this clock as its reference (202.5 MHz, generated),
// and the MCU's SPI clock is asynchronous to all of it and crosses
// through the handshake in mcu_spi.v.  PK8000 Nano's three lines, with
...
create_clock -name clk40 -period 24.691 -waveform {0 12.345} [get_pins {pll/rpll_inst/CLKOUT}]
...
// raises spi_data_in_ready, and clk40 reads it two flops after seeing
// that flag, by which time it has been stable for three SPI bit times.
set_clock_groups -asynchronous -group [get_clocks {spi_clk}] -group [get_clocks {clk27 clk40}]

//------------------------------------------------------------------------
// Left undone, in the order worth doing: set_input_delay/set_output_delay
// on the SDRAM bus (40.5 MHz with a 90-degree output clock: sdram.v's
