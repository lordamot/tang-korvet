# Korvet Nano - the ПК8020 "Корвет" on the Tang Nano 20K.
#
...
#   make sim         run the machine to the ОПТС's start screen (RUN_MS=1500)
...
#   make rom         regenerate tang/src/korvet/{opts_rom,font_rom}.v from tang/rom/
#   make memmap      regenerate tang/src/korvet/memmap.v (tools/decplm.py)
...
ROMHEX   := $(BUILD)/mif/opts_rom.hex $(BUILD)/mif/font_rom.hex $(BUILD)/mif/korvet11.hex

# The ОПТС runs its tests and reaches its start screen in about a second.
RUN_MS   ?= 1500
PPM_MAX  ?= 4
PPM_FROM ?= 1200
WAVE_MS  ?= 2
...
.PHONY: all toolchain lint sim wave frames fw rom memmap mif clean bitstream \
        flash-fpga flash-fpga-flash flash-mcu help menu-test extrom-test timing
...
#-----------------------------------------------------------------------
# ROM images.  The synthesised ROMs are pROM primitives, made by
# bin2prom.py and committed; the simulation models read $readmemh hex of
# the same bytes.
#-----------------------------------------------------------------------
mif: $(ROMHEX)

$(BUILD)/mif/opts_rom.hex: tang/rom/korvet20.rom $(TOOLS)/mif.py
	@mkdir -p $(dir $@)
	$(PYTHON) $(TOOLS)/mif.py binhex $< $@

$(BUILD)/mif/korvet11.hex: tang/rom/korvet11.rom $(TOOLS)/mif.py
	@mkdir -p $(dir $@)
	$(PYTHON) $(TOOLS)/mif.py binhex $< $@

$(BUILD)/mif/font_rom.hex: tang/rom/korvet2.fnt $(TOOLS)/mif.py
	@mkdir -p $(dir $@)
	$(PYTHON) $(TOOLS)/mif.py binhex $< $@

rom:
	$(PYTHON) $(TOOLS)/bin2prom.py tang/rom/korvet20.rom tang/src/korvet/opts_rom.v -m opts_rom
	$(PYTHON) $(TOOLS)/bin2prom.py tang/rom/korvet2.fnt tang/src/korvet/font_rom.v -m font_rom

memmap:
	$(PYTHON) $(TOOLS)/decplm.py --verilog
...
# Frames as .ppm, decoded back out of the TMDS stream: 1024x512.
frames: $(SIMBIN) mif
	@mkdir -p sim/out
...
bitstream:
	...
	@mkdir -p bin
	@rm -f bin/tang.fs && cp tang/impl/pnr/korvet.fs bin/tang.fs
	@echo
	@echo "bitstream: bin/tang.fs"
	@ls -l bin/tang.fs
	@echo "resources and timing:"
	@grep -iE "Timing Constraints|Logic|Register|BSRAM|PLL" \
	    tang/impl/pnr/korvet.rpt.txt 2>/dev/null | head -12 || true
...
# Korvet form and leaves each screen under build/menu/ as text and PNG.
...
# The ExtROM controller's state machine (mnano/extrom.c) on the host,
# against a directory standing in for the card: the phase-1 handshake,
# the API's commands, sector reads and writes of a .kdi.
extrom-test: mnano/extrom.c mnano/extrom_test.c mnano/extrom.h soft/disk.kdi
	@mkdir -p $(BUILD)/menu
	$(CC) -O -Wall -DEXTROM_HOST_TEST -Imnano -o $(BUILD)/menu/extrom_test mnano/extrom.c mnano/extrom_test.c
	$(BUILD)/menu/extrom_test $(BUILD)/menu/extrom soft/disk.kdi tang/rom/stage1.rom
	@echo "extrom-test: ok"
