...
Reads tang/impl/pnr/korvet_tr_content.html and korvet.log and exits
non-zero if the layout is one that could break the machine the way the
2 Sep 2026 layouts did (.claude/rules/timing.md here):
...
Korvet Nano has one internal clock (clk30) and nothing derived from a
counter bit, so there is no artefact to allow for.  The only exceptions
are the named ones below, inherited from UKNC Nano, each with a reason
and a slack floor; none has fired here.
...
TR   = PNR / "korvet_tr_content.html"
LOG  = PNR / "korvet.log"

REQUIRED_CLOCKS = ["clk27", "clk40", "spi_clk"]
KNOWN_FABRIC_CLOCKS = {"m0s_in[3]", "clk27_d"}   # the SPI clock pin; clk27 sampled in sdcmd_ctrl.v (MiSTeryNano's)
